...
export default function BinancePaperBot() {
  ...
  const metrics = useMemo(() => getPaperBotPerformanceMetrics(activeOrders, activeAccount?.equity, activeAccount?.initialCapital), [activeOrders, activeAccount?.equity, activeAccount?.initialCapital]);
  ...
  const handleEnableBot = async () => {
    try {
      await ensureSupabaseAccessToken();
      await save.mutateAsync({ symbols: parsedSymbols, strategy, scheduleMinutes: interval, riskPct: 1, dailyLossStopPct: 3, maxOpenPositions: 3, enabled: true, tradingMode });
  ...
  const equityPnl = activeAccount ? activeAccount.equity - (activeAccount?.initialCapital ?? 50) : 0;
  const equityPct = activeAccount?.initialCapital ? (equityPnl / activeAccount.initialCapital) * 100 : 0;
    ...
    <section className="binance-card paper-bot-card" id="binance-paper-bot">
      ...
      <div className="trading-mode-toggle-wrap">
        <div className="trading-mode-selector">
            ...
            onClick={() => {
              if (tradingMode !== "live") {
                const confirmed = window.confirm(
                  "⚠️ SWITCH TO LIVE SPOT TRADING?\n\nWhen Live mode is enabled, the DeepSeek AI quantitative bot will place REAL spot orders on your Binance account using real funds.\n\nMake sure your BINANCE_API_KEY and BINANCE_API_SECRET are set in your .env file.\n\nProceed to switch to Live Trading?"
                );
                if (confirmed) {
                  setTradingMode("live");
          ...
          >
            <span className="live-dot" /> ⚡ Binance Live Trading
          </button>
        </div>
        {isLive && (
          <div className={`live-credentials-banner ${credCheck.data?.ok ? "connected" : "warning"}`}>
            <ShieldCheck size={13} />
            {credCheck.data?.ok
              ? `Binance API Connected (${credCheck.data.testnet ? "Testnet" : "Spot Live"})`
              : (credCheck.data?.error ?? "API Key not configured in .env")}
          </div>
        )}
      </div>
      ...
      {isLive && (
        <div className="live-mode-disclaimer">
          <ShieldAlert size={15} />
          <div>
            <b>LIVE SPOT TRADING ACTIVE:</b> Real market orders will be placed on your Binance account following DeepSeek AI quantitative analysis and ATR risk management. Take-profit (+0.65%), stop-loss (-0.38%), and dynamic trailing profit locks execute directly on live Binance spot.
          </div>
        </div>
      )}
      ...
      <div className="paper-bot-banner">
        <ShieldCheck size={15} />
        <div>
          <b>{isLive ? "Live Binance Quantitative Trading Engine." : "Simulated quantitative trading engine."} Kelly confidence-scaled risk sizing &amp; ATR stops.</b>
      ...
      <div className={`paper-performance-board${inProfit ? " profit" : " loss"}`}>
        ...
        <div className="perf-stats-grid">
          ...
          <div><span>MAX DRAWDOWN</span><b className={metrics.maxDrawdownPct > 1 ? "down" : ""}>{metrics.maxDrawdownPct > 0 ? `${metrics.maxDrawdownPct.toFixed(2)}%` : "—"}</b><small>vs initial {money(activeAccount?.initialCapital)}</small></div>
        </div>
      </div>
