...
async function fetchSnapshot(symbol: string, key: string): Promise<MarketQuote> { try { const response = await fetchWithTimeout(`${baseUrl}/v2/snapshot/locale/us/markets/stocks/tickers/${encodeURIComponent(symbol)}?apiKey=${encodeURIComponent(key)}`); if (!response.ok) { if (response.status === 401 || response.status === 403) return fallbackQuote(symbol, `Massive snapshot access denied (${response.status}); using simulated feed`); throw new Error(`Massive snapshot request failed for ${symbol}: ${response.status}`); } const body = await response.json() as { ticker?: { ticker?: string; lastTrade?: { p?: number; t?: number }; min?: { c?: number; v?: number; av?: number; vw?: number }; day?: { h?: number; l?: number; c?: number; v?: number }; todaysChangePerc?: number } }; const t = body.ticker; const price = t?.lastTrade?.p ?? t?.day?.c ?? 0; const volume = t?.day?.v ?? t?.min?.av ?? t?.min?.v ?? 0; return { symbol: t?.ticker ?? symbol, price, bid: price, ask: price, changePct: t?.todaysChangePerc ?? 0, volume, rvol: 0, floatM: 0, marketCapM: 0, dollarVolumeM: (price * volume) / 1_000_000, vwap: t?.min?.vw ?? price, sessionHigh: t?.day?.h ?? price, sessionLow: t?.day?.l ?? price, halted: false, lastUpdated: t?.lastTrade?.t ?? Date.now(), source: "massive" }; } catch (error) { const reason = error instanceof Error ? error.message : "Massive snapshot network failure"; return fallbackQuote(symbol, `${reason}; using simulated feed`); } }
...
export class MassiveMarketDataProvider implements MarketDataProvider {
  async getQuotes(symbols: string[]) {
    ...
    const fallbackCount = results.filter(item => item.source === "simulated").length;
    await updateProviderHealth({ provider: "massive", status: fallbackCount === 0 ? "healthy" : fallbackCount === results.length ? "degraded" : "degraded", latencyMs: Date.now() - started, error: fallbackCount ? `${fallbackCount}/${results.length} snapshot requests used fallback` : undefined });
    return results;
  }

  async getTrades(symbol: string, from: string, to: string): Promise<MarketTrade[]> { const key = requireKey(); const response = await fetchWithTimeout(`${baseUrl}/v3/trades/${encodeURIComponent(symbol)}?timestamp.gte=${encodeURIComponent(from)}&timestamp.lte=${encodeURIComponent(to)}&limit=1000&apiKey=${encodeURIComponent(key)}`); if (!response.ok) throw new Error(`Massive trades request failed for ${symbol}: ${response.status}`); const body = await response.json() as { results?: Array<{ participant_timestamp?: number; price: number; size: number }> }; return (body.results ?? []).map(item => normalizeTrade({ ev: "T", sym: symbol, p: item.price, s: item.size, t: item.participant_timestamp })); }
