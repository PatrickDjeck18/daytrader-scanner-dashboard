...
async function fetchSnapshot(symbol: string, key: string): Promise<MarketQuote> { if (realtimeAccessDenied()) return fallbackQuote(symbol, "Massive Stocks plan does not include real-time snapshots"); try { const response = await fetchWithTimeout(`${baseUrl}/v2/snapshot/locale/us/markets/stocks/tickers/${encodeURIComponent(symbol)}?apiKey=${encodeURIComponent(key)}`); if (!response.ok) { if (response.status === 401 || response.status === 403) { markRealtimeDenied(); return fallbackQuote(symbol, `Massive Stocks plan does not include real-time snapshots (${response.status})`); } throw new Error(`Massive snapshot request failed for ${symbol}: ${response.status}`); } return normalizeRestSnapshot(await response.json() as RestSnapshot, symbol); } catch (error) { const reason = error instanceof Error ? error.message : "Massive snapshot network failure"; return fallbackQuote(symbol, reason); } }
...
export class MassiveMarketDataProvider implements MarketDataProvider {
  async getQuotes(symbols: string[]) {
    ...
    const planRestricted = results.some(item => item.providerError?.includes("plan does not include")); await updateProviderHealth({ provider: "massive", status: fallbackCount === 0 ? "healthy" : fallbackCount === results.length ? "offline" : "degraded", latencyMs: Date.now() - started, error: planRestricted ? "Stocks Basic does not include real-time snapshots" : fallbackCount ? `${fallbackCount}/${results.length} snapshot requests failed` : undefined });
    return results;
  }
