...
export const appRouter = router({
  ...
  binancePaper: router({
    ...
    saveBotConfig: auditedProtectedProcedure.input(z.object({ symbols: z.array(z.string().regex(/^[A-Z0-9]{5,24}$/)).min(1).max(6), strategy: z.enum(["scalp_momentum", "fast_momentum", "range_reversion", "learning_mode"]).default("scalp_momentum"), scheduleMinutes: z.union([z.literal(1), z.literal(5), z.literal(15)]), riskPct: z.number().positive().max(2), dailyLossStopPct: z.number().positive().max(5), maxOpenPositions: z.number().int().min(1).max(3), confidenceThreshold: z.number().min(0).max(1).optional(), stopLossPct: z.number().positive().max(2).optional(), takeProfitRatio: z.number().positive().max(5).optional(), momentumThreshold: z.number().positive().max(1).optional(), rangeUpperBound: z.number().positive().max(2).optional(), rangeLowerBound: z.number().positive().max(1).optional() })).mutation(({ ctx, input }) => savePaperBotConfig(ctx.user.id, input)),
