...
import { runUserPaperBotNow } from "./paper-bot-runner";
...
export const appRouter = router({
  ...
  crypto: router({
    ...
    bars: publicProcedure.input(z.object({ market: z.enum(CRYPTO_MARKETS), symbol: z.string().trim().toUpperCase().regex(/^[A-Z0-9/_\-]{5,24}$/), interval: z.enum(CRYPTO_INTERVALS), limit: z.number().int().min(20).max(1000).default(120) })).query(async ({ ctx, input }) => { try { assertRateLimit(`crypto-bars:${clientKey(ctx.req)}`, 60, 60_000); } catch (error) { if (error instanceof TRPCError && error.code === "TOO_MANY_REQUESTS") return []; throw error; } try { return await fetchBinanceCryptoBars(input.market, input.symbol, input.interval, input.limit); } catch { return []; } }),
  ...
  binancePaper: router({
    ...
    liveAccount: auditedProtectedProcedure.input(z.object({ prices: z.record(z.string(), z.number()).default({}) })).query(async ({ input }) => {
      try {
        return await fetchBinanceLiveAccountSummary(input.prices);
      } catch (err) {
        throw new TRPCError({ code: "BAD_REQUEST", message: err instanceof Error ? err.message : "Failed to fetch Binance Live account" });
      }
    }),
    ...
    saveBotConfig: auditedProtectedProcedure.input(z.object({
      symbols: z.array(z.string().regex(/^[A-Z0-9]{5,24}$/)).min(1).max(6),
      strategy: z.enum(["scalp_momentum", "fast_momentum", "range_reversion", "vwap_pullback", "bb_squeeze"]).default("scalp_momentum"),
      scheduleMinutes: z.union([z.literal(1), z.literal(5), z.literal(15)]),
      riskPct: z.number().positive().max(2),
      dailyLossStopPct: z.number().positive().max(5),
      maxOpenPositions: z.number().int().min(1).max(3),
      tradingMode: z.enum(["paper", "live"]).default("paper"),
    })).mutation(({ ctx, input }) => savePaperBotConfig(ctx.user.id, input)),
    enableBot: auditedProtectedProcedure.input(z.object({ scheduleMinutes: z.union([z.literal(1), z.literal(5), z.literal(15)]) })).mutation(async ({ ctx, input }) => {
      try {
        const config = await enableScheduledPaperBot(ctx.user.id, input.scheduleMinutes);
        // Trigger immediate first execution so orders/decisions happen right away
        runUserPaperBotNow(ctx.user.id).catch(err => console.warn("[PaperBot] Initial trigger on enable failed:", err));
        await safeAudit({ userId: ctx.user.id, action: "binance_paper_bot_enabled", resource: "binance-paper-bot", metadata: { scheduleMinutes: input.scheduleMinutes, mode: config.tradingMode || "paper" }, requestId: requestId(ctx.req) });
        return config;
      } catch (error) {
        throw new TRPCError({ code: "BAD_REQUEST", message: error instanceof Error ? error.message : "Paper bot could not be enabled" });
      }
    }),
    ...
    triggerBotNow: auditedProtectedProcedure.mutation(async ({ ctx }) => {
      try {
        const result = await runUserPaperBotNow(ctx.user.id);
        await safeAudit({ userId: ctx.user.id, action: "binance_paper_bot_manual_trigger", resource: "binance-paper-bot", metadata: { mode: "paper" }, requestId: requestId(ctx.req) });
        return result;
      } catch (error) {
        throw new TRPCError({ code: "BAD_REQUEST", message: error instanceof Error ? error.message : "Manual DeepSeek execution failed" });
      }
    }),
    resetAccount: auditedProtectedProcedure.mutation(async ({ ctx }) => {
      try {
        const result = await resetBinancePaperAccount(ctx.user.id);
        await safeAudit({ userId: ctx.user.id, action: "binance_paper_account_reset", resource: "binance-paper-account", metadata: { mode: "paper" }, requestId: requestId(ctx.req) });
        return result;
      } catch (error) {
        throw new TRPCError({ code: "BAD_REQUEST", message: error instanceof Error ? error.message : "Failed to reset paper account" });
      }
    }),
