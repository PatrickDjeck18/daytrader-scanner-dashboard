...
type MarketContext = { symbol: string; quote: { price: number | null; changePct: number | null; quoteVolume: number | null; availability: string }; oneMinute: { bars: number; changePct: number | null }; fiveMinute: { bars: number; changePct: number | null }; fifteenMinute: { bars: number; changePct: number | null } };
...
async function runPaperBotSymbol(input: { config: PaperBotConfig; taskUid: string; now: number; strategy: PaperBotStrategy; context: MarketContext; allContexts: MarketContext[]; prices: Record<string, number> }) {
  ...
  const started = await startPaperBotRun({ userId: config.userId, configId: config.id, runKey: paperBotRunKey(taskUid, config.id, config.scheduleMinutes, now, context.symbol), marketContext: { evaluationSymbol: context.symbol, strategy, contexts: allContexts, selectedContext: context } });
