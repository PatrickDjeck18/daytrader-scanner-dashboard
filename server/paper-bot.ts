...
export async function requestDeepSeekDecision(input: { marketContext: unknown; symbol: string; fetchImpl?: typeof fetch }): Promise<BotDecision> {
  ...
  const response = await fetchImpl("https://api.deepseek.com/chat/completions", { method: "POST", headers: { "content-type": "application/json", authorization: `Bearer ${apiKey}` }, body: JSON.stringify({ model: "deepseek-v4-flash", stream: false, max_tokens: 500, response_format: { type: "json_object" }, messages: [{ role: "system", content: "You analyze provider-returned Binance spot market context for a paper-trading simulation only. Return valid JSON with action, symbol, confidence, stopPrice, targetPrice, reason. action must be buy, sell, or hold. Never imply a real order, account action, leverage, transfer, or financial certainty." }, { role: "user", content: `Return JSON for symbol ${input.symbol}. Market context: ${JSON.stringify(input.marketContext)}` }] }), signal: AbortSignal.timeout(20_000) });
  if (!response.ok) throw new Error(`DeepSeek analysis request failed (${response.status})`);
  const payload = await response.json() as { choices?: Array<{ message?: { content?: string | null } }> };
  const content = payload.choices?.[0]?.message?.content;
  if (!content) throw new Error("DeepSeek returned no decision content");
  return botDecisionSchema.parse(JSON.parse(content));
}
