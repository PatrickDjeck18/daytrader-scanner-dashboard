...
export type NextCandleForecast = { direction: "up" | "down" | "flat"; probability: number; reason: string; timestamp: number };
...
export function getLatestNextCandleForecast(input: { symbol: string; runs: ActivityRun[] }): NextCandleForecast | undefined {
  for (const run of input.runs.slice().sort((left, right) => timestamp(right.createdAt) - timestamp(left.createdAt))) {
    ...
    if ((direction === "up" || direction === "down" || direction === "flat") && typeof probability === "number" && Number.isFinite(probability) && probability >= 0 && probability <= 1 && typeof reason === "string" && reason.trim()) return { direction, probability, reason, timestamp: timestamp(run.createdAt) };
  }
  return undefined;
}
...
function ActivityChart({ bars, markers }: { bars: CryptoBar[]; markers: PaperActivityMarker[] }) {
  ...
  const pricedMarkers = plotted.flatMap(marker => marker.price ? [marker.price] : []); const width = 860, height = 270, pad = 30, bottom = 225;
  const min = Math.min(...recent.map(bar => bar.low), ...pricedMarkers); const max = Math.max(...recent.map(bar => bar.high), ...pricedMarkers); const range = Math.max(0.00000001, max - min);
  const x = (at: number) => pad + ((at - recent[0].start) / Math.max(1, recent.at(-1)!.end - recent[0].start)) * (width - pad * 2); const y = (value: number) => bottom - ((value - min) / range) * (bottom - pad);
  ...
  const markerColor: Record<PaperActivityMarker["type"], string> = { entry: "#37d39b", stop: "#ef596f", target: "#8e74ff", hold: "#fcd535", risk_blocked: "#ef596f", error: "#f08ba2" }; const markerGlyph: Record<PaperActivityMarker["type"], string> = { entry: "E", stop: "S", target: "T", hold: "H", risk_blocked: "!", error: "×" };
...
export default function PaperBotActivityChart({ symbols, orders, runs }: { symbols: string[]; orders: ActivityOrder[]; runs: ActivityRun[] }) {
  const validSymbols = symbols.length ? symbols : ["BTCUSDT"]; const [symbol, setSymbol] = useState(validSymbols[0]);
  ...
  const markers = useMemo(() => buildPaperActivityMarkers({ symbol, orders, runs }), [symbol, orders, runs]); const forecast = useMemo(() => getLatestNextCandleForecast({ symbol, runs }), [symbol, runs]);
