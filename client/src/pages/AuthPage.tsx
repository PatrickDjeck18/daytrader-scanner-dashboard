...
import { ArrowRight, CheckCircle2, ChevronLeft, KeyRound, LoaderCircle, LockKeyhole, Mail, TriangleAlert } from "lucide-react";
import { useLocation } from "wouter";
import { supabase } from "@/lib/supabase";
import { useSupabaseAuth } from "@/contexts/SupabaseAuthContext";

type AuthMode = "sign-in" | "recover" | "update-password";

function redirectPath(search: string) {
  const next = new URLSearchParams(search).get("next");
  // Default landing page after login is the Binance crypto dashboard.
  return next?.startsWith("/") && !next.startsWith("//") ? next : "/binance";
}
...
export default function AuthPage({ initialMode = "sign-in" }: { initialMode?: AuthMode }) {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (mode === "update-password" && password !== confirmPassword) { setError("Passwords do not match."); return; }
    setSubmitting(true);
    try {
      if (mode === "sign-in") {
  ...
  const title = mode === "sign-in" ? "Welcome back" : mode === "recover" ? "Reset access" : "Set a new password";
  const description = mode === "sign-in" ? "Sign in to unlock your private dashboards and saved workspace." : mode === "recover" ? "We will send a secure reset link if an account exists for this address." : "Your reset link is verified. Choose a new secure password.";

  return <main className="auth-shell"><section className="auth-brief"><div className="auth-brand"><span>◆</span> ARCANE<small>MONITOR</small></div><div><p className="auth-eyebrow">PRIVATE WORKSPACE</p><h1>One secure session.<br />Two focused dashboards.</h1><p>Access the U.S. equities scanner and Binance crypto terminal with Supabase authentication and user-isolated saved data.</p></div><div className="auth-assurance"><div><LockKeyhole size={16} /><span><b>Private workspace data</b><small>Layouts, watchlists, alerts, and paper history are scoped to your account.</small></span></div></div></section><section className="auth-panel"><div className="auth-card"><div className="auth-card-heading"><p className="auth-eyebrow">SECURE ACCESS</p><h2>{title}</h2><p>{description}</p></div>{message ? <div className="auth-message success"><CheckCircle2 size={16} />{message}</div> : null}{error ? <div className="auth-message error"><TriangleAlert size={16} />{error}</div> : null}<form onSubmit={submit}><label>Email{mode !== "update-password" ? <span className="auth-input"><Mail size={15} /><input type="email" autoComplete="email" value={email} onChange={event => setEmail(event.target.value)} placeholder="you@example.com" required /></span> : null}</label>{mode !== "recover" ? <label>{mode === "update-password" ? "New password" : "Password"}<span className="auth-input"><KeyRound size={15} /><input type="password" autoComplete={mode === "update-password" ? "new-password" : "current-password"} value={password} onChange={event => setPassword(event.target.value)} placeholder="At least 8 characters" required /></span></label> : null}{mode === "update-password" ? <label>Confirm password<span className="auth-input"><KeyRound size={15} /><input type="password" autoComplete="new-password" value={confirmPassword} onChange={event => setConfirmPassword(event.target.value)} placeholder="Repeat your password" required /></span></label> : null}<button className="auth-submit" type="submit" disabled={submitting}>{submitting ? <LoaderCircle className="spin" size={16} /> : <ArrowRight size={16} />}{mode === "sign-in" ? "Sign in" : mode === "recover" ? "Send reset link" : "Update password"}</button></form>{mode === "sign-in" ? <div className="auth-links"><button type="button" onClick={() => switchMode("recover")}>Forgot password?</button></div> : <div className="auth-links"><button type="button" onClick={() => switchMode("sign-in")}><ChevronLeft size={13} />Back to sign in</button></div>}<p className="auth-footnote">By continuing, you access paper-only market tools. No live orders are available.</p></div></section></main>;
}
