...
function HistoricalBars({ bars }: { bars: Array<{ timestamp: number; open: number; high: number; low: number; close: number; volume: number }> }) {
  const recent = bars.slice(-48); const width = 760, height = 240, pad = 22; const lows = recent.map(b => b.low), highs = recent.map(b => b.high); const min = Math.min(...lows), max = Math.max(...highs), range = Math.max(.01, max - min); const x = (i: number) => pad + (i / Math.max(1, recent.length - 1)) * (width - pad * 2); const y = (value: number) => height - pad - ((value - min) / range) * (height - pad * 2); const points = recent.map((bar, i) => `${x(i)},${y(bar.close)}`).join(" ");
  return <div className="history-chart"><div className="history-label">PERMITTED HISTORICAL BARS · NON-LIVE</div>{recent.length === 0 ? <div className="panel-state"><BookOpen size={15} /><span>No historical bars returned for this range.</span></div> : <><div className="chart-readout"><span>OHLC</span><b>${recent[recent.length - 1].open.toFixed(2)} / ${recent[recent.length - 1].high.toFixed(2)} / ${recent[recent.length - 1].low.toFixed(2)} / ${recent[recent.length - 1].close.toFixed(2)}</b></div><svg viewBox={`0 0 ${width} ${height}`} className="market-chart" role="img" aria-label="Provider-backed historical price chart"><defs><linearGradient id="chart-fill" x1="0" x2="0" y1="0" y2="1"><stop offset="0%" stopColor="#8175ff" stopOpacity=".28" /><stop offset="100%" stopColor="#8175ff" stopOpacity="0" /></linearGradient></defs><path d={`M ${points} L ${x(recent.length - 1)},${height - pad} L ${x(0)},${height - pad} Z`} fill="url(#chart-fill)" /><polyline points={points} fill="none" stroke="#9a8cff" strokeWidth="2.5" strokeLinejoin="round" strokeLinecap="round" />{recent.map((bar, i) => <line key={bar.timestamp} x1={x(i)} x2={x(i)} y1={y(bar.high)} y2={y(bar.low)} stroke={bar.close >= bar.open ? "#45d6a5" : "#f2768e"} strokeWidth="1.5" />)}</svg><div className="chart-axis-labels"><span>${max.toFixed(2)}</span><span>${((max + min) / 2).toFixed(2)}</span><span>${min.toFixed(2)}</span></div></>}</div>;
}
