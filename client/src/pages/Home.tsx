...
export default function Home() {
  ...
  const feedStale = Boolean(liveQuotes.data?.some(q => q.lastUpdated > 0 && Date.now() - q.lastUpdated > 60_000));
  const feedWarning = liveQuotes.isError || hasFallbackQuotes || feedStale || providerHealth.data?.status === "degraded" || providerHealth.data?.status === "offline";
  ...
  const scannerRows = useMemo(() => {
    ...
    const sortBy = (key: "change" | "rvol" | "volume" | "floatM" | "spread") => [...eligible].sort((a, b) => b[key] - a[key]);
    if (scanner === "High-of-Day Breakout" || scanner === "Opening Range Breakout") return eligible.filter(s => s.price >= s.high * (scanner === "High-of-Day Breakout" ? .985 : .97));
    if (scanner === "Relative Volume Leaders" || scanner === "Unusual Tape Activity") return sortBy("rvol");
    if (scanner === "Low-Float Momentum") return [...eligible].filter(s => s.floatM < 500).sort((a, b) => b.change - a.change);
    if (scanner === "Pre-Market Movers") return [...eligible].sort((a, b) => b.premarket - a.premarket);
    if (scanner === "VWAP Reclaim/Loss") return eligible.filter(s => s.price >= s.vwap).sort((a, b) => (b.price - b.vwap) - (a.price - a.vwap));
    if (scanner === "Halt Monitor") return [...eligible].filter((_, i) => i === 2).sort((a, b) => b.change - a.change);
    if (scanner === "Offering/Dilution Risk") return [...eligible].filter(s => s.catalystType === "Earnings" || s.catalystType === "News").sort((a, b) => a.change - b.change);
    return sortBy("change");
  }, [stocks, scanner, thresholds]);
    ...
    <main className="dashboard-grid">
      ...
      <div className="main-column">
        ...
        <div className="lower-grid">
          ...
          <Panel title="Live chart" subtitle={`${stock.symbol} · 1 minute`} className="chart-panel" action={<><button className="timeframe active">1m</button><button className="timeframe">5m</button><button className="timeframe">D</button><button className="icon-btn"><Maximize2 size={13} /></button></>}><CandleChart stock={stock} /></Panel>
        </div>

        <div className="bottom-grid">
          <Panel title="Catalyst feed" subtitle="News & filings · newest first" className="news-panel" action={<button className="filter-chip">All sources <ChevronDown size={11} /></button>}><div className="news-list">{newsItems.map(item => <div className="news-row" key={item.time + item.symbol}><span className="news-time">{item.time}</span><div className={`news-type ${item.tone}`} /> <div className="news-copy"><div><b>{item.symbol}</b><span className={`news-tag ${item.tone}`}>{item.type}</span></div><p>{item.title}</p><small>{item.source}</small></div><button className="open-news">↗</button></div>)}</div></Panel>
          <Panel title="Watchlist" subtitle={`${watchlist.length} symbols · custom view`} className="watch-panel" action={<button className="filter-chip" onClick={() => setWatchColumns(watchColumns.length === 3 ? ["LAST", "CHG"] : ["LAST", "CHG", "ALERT"])}><Settings2 size={12} /> Columns</button>}><div className="watch-table"><div className="watch-head"><span>SYMBOL</span>{watchColumns.map(column => <span key={column}>{column}</span>)}</div>{watchlist.map(ticker => { const s = stocks.find(x => x.symbol === ticker) ?? stocks[0]; return <button draggable className="watch-row" key={ticker} onDragStart={() => setDraggedTicker(ticker)} onDragOver={e => e.preventDefault()} onDrop={() => { if (draggedTicker && draggedTicker !== ticker) { const next = [...watchlist]; const from = next.indexOf(draggedTicker), to = next.indexOf(ticker); next.splice(from, 1); next.splice(to, 0, draggedTicker); setWatchlist(next); } setDraggedTicker(null); }} onClick={() => setSelected(ticker)}><span><span className="watch-star"><Star size={11} fill="currentColor" /></span><b>{ticker}</b></span>{watchColumns.includes("LAST") && <span>${s.price.toFixed(2)}</span>}{watchColumns.includes("CHG") && <span className="positive">+{s.change.toFixed(1)}%</span>}{watchColumns.includes("ALERT") && <span className={`alert-state ${muted.includes(ticker) ? "muted" : "on"}`}>{muted.includes(ticker) ? <BellOff size={11} /> : <Bell size={11} />}</span>}</button>; })}</div><button className="add-watch"><Plus size={13} /> Add symbol</button></Panel>
        </div>
      </div>
