      ...
      <div className="main-column">
        ...
        <div className="lower-grid">
          <Panel title="Symbol workspace" subtitle="Selected symbol · live quote" className="detail-panel" action={<span className="panel-kbd">⌘ K</span>}>
            ...
            <div className="symbol-hero"><div className="hero-title"><span className="hero-symbol">{stock.symbol}</span><span className="hero-name">{stock.name}</span><div className="hero-badges"><span className="badge badge-purple">{stock.catalystType}</span><span className="badge badge-green">{stock.sector}</span></div></div><div className="hero-quote"><PriceDirection change={stock.change} value={`$${stock.price.toFixed(2)}`} /><PriceDirection change={stock.change} value={formatSignedPercent(stock.change)} /><small className={getPriceDirection(stock.change) === "down" ? "negative" : "positive"}>{stock.change > 0 ? "+" : ""}{(stock.price * stock.change / 100).toFixed(2)}</small></div></div>
