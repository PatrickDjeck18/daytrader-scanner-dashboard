    ...
    <main className="dashboard-grid">
      ...
      <div className="main-column">
        ...
        <div className="lower-grid">
          ...
          <Panel title="Live chart" subtitle={`${stock.symbol} · 1 minute`} className="chart-panel" action={<><button className="timeframe active">1m</button><button className="timeframe">5m</button><button className="timeframe">D</button><button className="icon-btn"><Maximize2 size={13} /></button></>}>{dataUnavailable && !demoMode ? <div className="panel-state unavailable-state"><Radio size={15} /><span>LIVE DATA UNAVAILABLE — chart withheld to prevent stale or seeded values.</span></div> : <CandleChart stock={stock} />}</Panel>
        </div>

        <div className="bottom-grid">
          <Panel title="Catalyst feed" subtitle="News & filings · newest first" className="news-panel" action={<button className="filter-chip">All sources <ChevronDown size={11} /></button>}>{dataUnavailable && !demoMode ? <div className="panel-state unavailable-state"><Radio size={15} /><span>LIVE NEWS UNAVAILABLE — no seeded headlines shown.</span></div> : <div className="news-list">{displayNews.map(item => (<div className="news-row" key={item.time + item.symbol}><span className="news-time">{item.time}</span><div className={`news-type ${item.tone}`} /><div className="news-copy"><div><b>{item.symbol}</b><span className={`news-tag ${item.tone}`}>{item.type}</span></div><p>{item.title}</p><small>{item.source}</small></div><button className="open-news">↗</button></div>))}</div>}</Panel>
