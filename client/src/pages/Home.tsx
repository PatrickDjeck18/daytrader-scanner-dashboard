...
export default function Home() {
  ...
  useEffect(() => { if (!liveQuotes.data?.length) return; setStocks(current => current.map(seed => { const quote = liveQuotes.data.find(item => item.symbol === seed.symbol); if (!quote || !isLiveProviderSource(quote.source)) return seed; return { ...seed, price: quote.price, change: quote.changePct, volume: quote.volume / 1000000, rvol: quote.source === "finnhub" ? 0 : seed.rvol, float: quote.source === "finnhub" ? "—" : seed.float, floatM: quote.source === "finnhub" ? 0 : seed.floatM, marketCap: quote.source === "finnhub" ? "—" : seed.marketCap, catalyst: quote.source === "finnhub" ? "Finnhub quote" : seed.catalyst, catalystType: quote.source === "finnhub" ? "Quote" : seed.catalystType, vwap: quote.vwap, high: quote.sessionHigh, low: quote.sessionLow, spread: Math.max(.01, quote.ask - quote.bid) }; })); }, [liveQuotes.data]);
