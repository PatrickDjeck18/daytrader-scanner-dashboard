...
export default function Home() {
  ...
  const liveDataReady = Boolean(liveQuotes.data?.some(q => q.source === "massive"));
  const dataUnavailable = !demoMode && !liveDataReady;
  const feedStale = Boolean(liveQuotes.data?.some(q => q.source === "massive" && q.lastUpdated > 0 && Date.now() - q.lastUpdated > 60_000));
  ...
  const displayStocks = demoMode ? stocks : stocks.filter(s => liveQuotes.data?.some(q => q.symbol === s.symbol && q.source === "massive"));
  ...
  useEffect(() => { if (!liveQuotes.data?.length) return; setStocks(current => current.map(seed => { const quote = liveQuotes.data.find(item => item.symbol === seed.symbol); return quote && quote.source === "massive" ? { ...seed, price: quote.price, change: quote.changePct, volume: quote.volume / 1000000, vwap: quote.vwap, high: quote.sessionHigh, low: quote.sessionLow, spread: Math.max(.01, quote.ask - quote.bid) } : seed; })); }, [liveQuotes.data]);
  const scannerRows = useMemo(() => {
    ...
    const eligible = displayStocks.filter(s => s.price >= minPrice && s.floatM >= minFloat && s.floatM <= maxFloat && Number(s.marketCap.replace(/[$TB]/g, "")) * (s.marketCap.includes("T") ? 1000000 : s.marketCap.includes("B") ? 1000 : 1) >= minCap && Number(s.volume) * s.price >= minDollar && s.change >= minChange && s.rvol >= minRvol && s.spread <= maxSpread);
  ...
  }, [displayStocks, scanner, thresholds]);
  ...
  return <div className="terminal-app">
    ...
    <main className="dashboard-grid">
      ...
      <aside className="right-column">
        ...
        <div className="right-footer"><button><Mic2 size={13} /> Voice search</button><span>{demoMode ? "Data is simulated (explicit demo mode)" : dataUnavailable ? "Live data unavailable" : "Massive live data"}</span></div>
      </aside>
    </main>
    <div className="toast"><span className="status-dot" /> {demoMode ? "Explicit demo feed connected" : dataUnavailable ? "Live data unavailable" : "Massive feed connected"} <button><X size={13} /></button></div>
  </div>;
}
