...
export const binancePaperAccounts = pgTable("binancePaperAccounts", {
  id: serial("id").primaryKey(), userId: integer("userId").notNull().unique(), initialCapital: numeric("initialCapital", { precision: 18, scale: 2 }).notNull().default("50.00"), currency: varchar("currency", { length: 12 }).notNull().default("USDT"), dailyAnchor: varchar("dailyAnchor", { length: 10 }).notNull().default("1970-01-01"), dailyStartEquity: numeric("dailyStartEquity", { precision: 18, scale: 2 }).notNull().default("50.00"), createdAt: timestamp("createdAt", { withTimezone: false }).defaultNow().notNull(), updatedAt: timestamp("updatedAt", { withTimezone: false }).defaultNow().notNull(),
...
export const binanceLiveOrders = pgTable("binanceLiveOrders", {
  ...
  clientOrderId: varchar("clientOrderId", { length: 96 }),
  symbol: varchar("symbol", { length: 24 }).notNull(),
  side: orderSide("side").notNull(),
  orderType: varchar("orderType", { length: 24 }).notNull().default("MARKET"),
  quantity: numeric("quantity", { precision: 22, scale: 8 }).notNull(),
  fillPrice: numeric("fillPrice", { precision: 22, scale: 8 }).notNull(),
  cummulativeQuoteQty: numeric("cummulativeQuoteQty", { precision: 22, scale: 8 }),
  status: varchar("status", { length: 32 }).notNull().default("FILLED"),
  source: varchar("source", { length: 32 }).notNull().default("deepseek-live-bot"),
  rawResponse: text("rawResponse"),
  createdAt: timestamp("createdAt", { withTimezone: false }).defaultNow().notNull(),
}, table => ({
  userIdIdx: index("binanceLiveOrders_userId_idx").on(table.userId),
  symbolIdx: index("binanceLiveOrders_symbol_idx").on(table.symbol),
  orderIdIdx: index("binanceLiveOrders_orderId_idx").on(table.orderId),
}));
